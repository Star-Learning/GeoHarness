...
test('Scenario 04 is an independent road-accessibility Demo package', async () => {
  ...
  assert.equal(scenario.data.roads.features.filter(feature => feature.properties.road_class === 'major').length, 51)
  assert.equal(scenario.result.expected.accessible_buildings_count, 249)
