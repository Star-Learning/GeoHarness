...
2026-08-31 的“下曼哈顿消防覆盖盲区”Topic 与 1080p60 录屏没有活跃阻塞。Provider 在长会话中
出现过可恢复的 `TRANSPORT` 重试；Agent 的真实 Tool 结果、15 个 Layer、26 栋未覆盖建筑与两项
导出均已完成，并由独立 oracle 核验。原始桌面录屏后段因前台窗口被切到其他应用而不可交付，成片
已删除全部非 GeoHarness 区间，只保留两个连续真实 Agent 区间；该编辑事实与精确时间范围记录在
`examples/topics/02-firehouse-coverage/media/recording.json`，不是产品代码阻塞。

2026-09-01 的全球卫星底图预览没有活跃代码阻塞。Esri World Imagery 已在真实 Harness 页面加载，
Web Mercator 对齐、三种底图切换、高清缩放和拖动均通过浏览器验收。它是需要网络的第三方显示服务，
可用性和许可受 Esri 条款约束；断网时 GeoHarness 会显示本地底色，canonical Layer、Agent Tool 和
分析结果不受影响。正式发布前仍应确定自有 MapTiler/天地图/高德 Provider 与 Key 管理策略。

2026-09-01 的地图/Agent Workspace 版式优化没有活跃阻塞。真实 1120×912 Harness 页面确认地图
主视区扩大、Layers/Legend 默认不遮挡、角色透明度生效，右侧完整 Agent Stream 与原生 composer
仍可用。图层显示强度只是浏览器合成规则，不回写 canonical 数据，也不会改变测试结果。

2026-09-01 的新会话自动定位代码没有活跃阻塞：空 Workspace 已在真实 Harness 页面自动发起浏览器
定位请求。当前电脑的 Windows 系统位置服务进程虽然运行，但当前用户位置隐私值为 `Deny`，所以真实
请求会超时；这是外部操作系统权限，不由 GeoHarness 静默修改。用户在“隐私和安全性 → 位置”开启
位置服务与桌面应用访问后，可新建空会话或点击地图定位按钮复验。
...
### 12. 卫星 RGB 视觉巡检的外部与解释边界

当前无代码阻塞。真实 Esri World Imagery 请求曾出现一次 `SSL: UNEXPECTED_EOF_WHILE_READING`，
Native Agent 在同一轮明确记录失败并重试成功；因此在线瓦片服务、TLS 和网络波动仍是运行时外部
依赖，不能通过预设统计掩盖。后端对单次视野限制为最多 16 张瓦片和最长边 768 px，失败会显式返回。

当前位置默认只存在浏览器；只有用户点击 `AI` 后才把 bbox/zoom 写入本地 Session 并请求 Esri。
本机定位视野的精确 bbox、截图和瓦片不进入仓库，Topic 的 `final.png` 使用公开纽约视野。RGB
颜色/边缘筛查不是辐射定标、多光谱分类、NDVI、变化检测或测地面积，这一能力边界是产品声明，
不是待修复缺陷。

地名分析现在同时依赖 Esri World Geocoding 候选和 OpenStreetMap Nominatim Polygon/MultiPolygon。
洪山区真实验收已用 OSM relation 3080399 的 702 个坐标点裁剪影像、Overlay 和像素统计，不再把
矩形候选 extent 当作行政区。OSM 边界是可审计的公开社区数据，不是中国官方法定行政区界线；
Nominatim 缺少多边形、限流或不可用时，GeoHarness 会显式回退到有界地图视野并披露该限制。
这属于外部数据来源与产品解释边界，不是当前代码阻塞。

2026-09-01 的渐进式巡检复验中，首个 `inspect_satellite_view` 再次遇到 Esri TLS
`UNEXPECTED_EOF_WHILE_READING`，Harness 保留失败 Tool Trace，Agent 随后自主重试成功。新的
`imagery/target` 快照让地图在瓦片下载期间已经完成定位、边界展示和加载反馈，但不能消除第三方瓦片
服务或本地网络的瞬时失败；重试与显式失败仍是正确行为，当前没有代码阻塞。

### 13. 2026-09-05 洪山区录制：外部查询与采集限制

- 本轮在线 Esri 地名查询发生 TLS EOF；独立 HTTPS/IPv4 诊断和 OSM 连通性检查超时。
  瓦片接口可访问。没有关闭 TLS 校验、修改网络安全策略或伪造地名/边界。
- 用户明确接受真实边界缓存后，指定会话导入 2026-09-01 的真实地名和 OSM relation 3080399 边界；
  两次缓存路径真实运行成功，均重新获取影像和计算分类。此方案不代表在线地名/边界服务已恢复。
  新地点仍需要在线解析或显式导入其真实缓存。后续分发地名缓存需核对 Esri 服务许可。
- 录制接口仅支持浏览器截图，非原生 60 fps 录像；一次前台采集超时，且中途源画幅变为 1440p。
  成片保留同一会话的真实素材，统一 1080p，60 fps 为重复帧导出；报告段做有标注的同帧局部放大。
  未采集等待段被明确省略，没有声称无间断实时录屏。原始帧、剪辑区间和来源尺寸可本地复核。
- 本次还发现并修复 canonical 读取阻塞前端进度轮询的真实代码问题；最终实录在工具尚未完成时已显示
  到达目标、边界与处理中反馈。它已解决，不应再把所有长时间无反馈归因于网络。
- 视觉初筛仍是 RGB 启发式而非经过验证的分割模型。Agent 报告包含未由工具单独验证的方位描述和
  推测性文字，发布说明已要求人工核查；不得把像素份额或启发式分数宣传为地物面积或识别准确率。

### 14. 2026-09-05 连续飞行的浏览器验收限制

连续相机/瓦片淡入已在本地实现，TypeScript、构建与真实边界回归可运行。浏览器成功刷新后，
截图请求及一次降为只读页面状态的检查均因自动审批服务 `502 Bad Gateway` 被拒绝；未通过其他
技术绕过。当前不能声称已测得实机 60 fps 或已完成视觉流畅度验收。该限制不影响代码与数值回归。
此前约 8 fps 截图源生成的 MP4 没有因本次前端修改自动更新；流畅视频仍需新的高帧率录制素材。
